...
$lp_ruangok    = $sections['lp_ruangok'] ?? [];
$ruang_operasi = $sections['ruang_operasi'] ?? [];
$resume        = $sections['resume'] ?? [];
$analisa       = $sections['analisa'] ?? [];
$lainnya       = $sections['lainnya'] ?? [];

$resume_penunjang = json_decode($resume['penunjang'] ?? '[]', true);
if (!is_array($resume_penunjang)) {
    $resume_penunjang = [];
}
if (empty($resume_penunjang) && !empty(trim((string)($resume['penunjang'] ?? '')))) {
    $resume_penunjang = [[
        'tipe'          => '',
        'tanggal'       => '',
        'hasil'         => $resume['penunjang'],
        'satuan'        => '',
        'nilai_rujukan' => '',
    ]];
}

$resume_terapi = json_decode($resume['terapi'] ?? '[]', true);
if (!is_array($resume_terapi)) {
    $resume_terapi = [];
}
if (empty($resume_terapi) && !empty(trim((string)($resume['terapi'] ?? '')))) {
    $resume_terapi = [[
        'jenis_obat'     => $resume['terapi'],
        'dosis'          => '',
        'kegunaan'       => '',
        'cara_pemberian' => '',
    ]];
...
<body>
    <div >
        ...
        <!-- HEADER -->
        <h1>FORMAT LAPORAN PENDAHULUAN MENGENAI RUANG OK</h1>
        <br>
